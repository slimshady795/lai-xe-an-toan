import { LeftOutlined, PauseOutlined, PlayCircleOutlined, RightOutlined, UndoOutlined } from '@ant-design/icons';
...
const Simulation = () => {
  ...
  const onPlay = (isPlay) => {
    if (isPlay) {
      time?.start === 0 && setTime({ duration: videoRef?.current?.duration, start: getTimeStamp(), pause: 0 })
  ...
  const disablePrev = question <= 1
  const disableNext = question >= questions?.length
  const questionInfo = questions?.[question - 1]

  return (
    <div className='simulation-page'>
      <div key={question} className='test'>
        <div className='title'>
        ...
        <video
          ...
          onLoadedData={e => setTime(prev => ({ ...prev, duration: e?.target?.duration }))}
        >
        ...
        <div className='control'>
          <div className='control-btn'>
            {isPlay
              ? <Button shape="default" size='large' icon={<PlayCircleOutlined />} onClick={() => onPlay(true)} />
              : <Button shape="default" size='large' icon={<PauseOutlined />} onClick={() => onPlay(false)} />
            }
            <Button shape="default" size='large' icon={<UndoOutlined />} onClick={onReplay} />
          </div>
          {time?.duration > 0 && <div className='control-time'>
            đáp án đúng từ giây {formatTime(+questionInfo.time1)} đến giây {formatTime(+questionInfo.time2)} - tổng video {formatTime(time.duration)} giây
          </div>}
        </div>
        ...
        {questions.map((_, idx) => {
          ...
          return (
            <>
              ...
              <div
                key={idx}
                className={clsx('question-item', qIdx === question && 'active')}
