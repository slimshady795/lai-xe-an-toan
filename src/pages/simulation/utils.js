...
export const CHAPTERS = [
    { title: 'Trên đường đô thị', questions: 29 },
    { title: 'Trên đường nông thôn', questions: 12 },
...
export const getVideo = idx => `https://taplai.net/videos/${idx}.mp4`

export const getTimeStamp = () => new Date().getTime()

export const formatTime = (time = 0, fixed = 3) => +(time).toFixed(fixed)

export const getStep = ({ time_1 = 0, time_2 = 0 }) => formatTime((time_2 - time_1) / RANGE)

export const getResultRange = (data = {}) => {
    const { time_1 = 0, time_2 = 0 } = data || {}

    const step = getStep({ time_1, time_2 })

    return [...Array(RANGE)].reduce((acc) => {
        const { start: lastStart } = acc?.[acc?.length - 1] || {}
        const start = formatTime(lastStart ? lastStart + step : time_1)
